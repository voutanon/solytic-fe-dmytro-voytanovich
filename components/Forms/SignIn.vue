<template>
  <form class="c-form">
    <IconTextInput v-model="v$.email.$model" :error-message="checkValidateError(v$.email)" placeholder="Username" icon="Login" />
    <IconTextInput v-model="v$.password.$model" :error-message="checkValidateError(v$.password)" placeholder="Password" icon="Password" type="password" />
    <AppButton @click="signIn" :disable="hasValidateErrors(v$)" title="SIGN IN" />
    <span class="c-form__text">Not a member? <nuxt-link to="#" class="c-form__link">Sign up now</nuxt-link></span>
  </form>
</template>
...
import { checkValidateError, hasValidateErrors } from "@/assets/mixins/VueValidateErrors"
